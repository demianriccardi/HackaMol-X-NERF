...
my %ires ;
my @groups = 
             $hack->group_by_atom_attr('resid',@fun);

foreach my $group (@groups){
  next unless ($group->count_atoms > 3);
  foreach my $atom ($group->all_atoms){
    hbuild_sp3($group,$atom);
  }
}
...
sub hbuild_sp3{
  ...
  my ($group, $atom) = (shift,shift);

  #find all the bonds to the atom of interest 
  my @bonds = $hack->find_bonds_brute(
                                  bond_atoms => [$atom],
                                  candidates => [$group->all_atoms],
                                  fudge      => 0.45,
                                  max_bonds  => 6,
  );
 
  if (@bonds > 3){
    print "already at least four bonds on atom ", $atom->iatom, "\n";
    return;
  }
  elsif(@bonds == 1){ 
